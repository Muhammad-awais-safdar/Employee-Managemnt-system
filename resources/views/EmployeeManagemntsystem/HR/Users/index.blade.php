                                    ...
                                    <table class="table table-bordered table-striped">
                                        <thead class="table-dark">
                                            <tr>
                                                ...
                                                <th class="text-white">Team Lead</th>
                                        ...
                                        <tbody>
                                            @forelse ($users as $user)
                                                <tr>
                                                    ...
                                                    <td>{{ $user->teamLead->name ?? 'N/A' }}</td>
                                                    <td>
                                                    <div class="btn-group" role="group">
                                                        <a href="{{ route('HR.users.show', $user->id) }}" class="btn btn-info btn-sm" title="View">
                                                            <i class="fas fa-eye"></i>
                                                        </a>
                                                        @if($canEdit)
                                                        <a href="{{ route('HR.users.edit', $user->id) }}" class="btn btn-primary btn-sm" title="Edit">
                                                            <i class="fas fa-edit"></i>
                                                        </a>
                                                        @endif
                                                        @if($canDelete && $user->id !== auth()->id())
                                                        <form action="{{ route('HR.users.destroy', $user->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Are you sure you want to delete this user?')">
                                                            @csrf
                                                            @method('DELETE')
