        ...
        <div class="card">
            <div class="card-body">
                <div class="d-flex align-items-center justify-content-between flex-wrap row-gap-2">
                    ...
                    <h5 class="fw-bold mb-0">Leave Types Management</h5>
                </div>
            </div>
        </div>
            ...
            <div class="card-body">
                <div class="d-flex align-items-center justify-content-between flex-wrap row-gap-2">
                    ...
                    <div class="d-flex gap-2">
                        <button class="btn btn-outline-success" onclick="createDefaults()">
                            <i class="ti ti-plus me-1"></i>Create Defaults
                        </button>
            ...
            <div class="card-body p-0">
                                ...
                                <td>
                                    <div class="form-check form-switch">
                                        <input class="form-check-input" type="checkbox" 
                                               ...
                                               onclick="toggleStatus('{{ $leaveType->id }}', this)">
                                ...
                                <td>
                                    <div class="dropdown">
                                        ...
                                        <ul class="dropdown-menu">
                                            <li>
                                                <a class="dropdown-item" href="{{ route('Admin.leave-types.show', $leaveType) }}">
                                                    <i class="ti ti-eye me-2"></i>View Details
                                                </a>
                                            </li>
                                            <li>
                                                <a class="dropdown-item" href="{{ route('Admin.leave-types.edit', $leaveType) }}">
                                                    <i class="ti ti-edit me-2"></i>Edit
                                                </a>
                                            </li>
                                            <li><hr class="dropdown-divider"></li>
                                            <li>
                                                <a class="dropdown-item text-danger" href="#" onclick="deleteLeaveType('{{ $leaveType->id }}', '{{ $leaveType->name }}')">
                ...
                <div class="text-center py-5">
                    <div class="empty-state">
                        ...
                        <p class="text-muted small mb-3">Get started by creating default leave types</p>
                        <div class="d-flex justify-content-center gap-2">
                            <button class="btn btn-outline-success" onclick="createDefaults()">
                                <i class="ti ti-plus me-1"></i>Create Defaults
                            </button>
                            <a href="{{ route('Admin.leave-types.create') }}" class="btn btn-primary">
                                <i class="ti ti-plus me-1"></i>Add Custom Type
                            </a>
                        </div>
                    </div>
                </div>
...
async function toggleStatus(leaveTypeId, checkbox) {
    const originalState = checkbox.checked;
    
    try {
        const response = await fetch(`{{ route('Admin.leave-types.index') }}/${leaveTypeId}/toggle-status`, {
            method: 'POST',
            headers: {
...
async function deleteLeaveType(leaveTypeId, leaveTypeName) {
    const result = await Swal.fire({
        title: 'Delete Leave Type?',
        text: `Are you sure you want to delete "${leaveTypeName}"? This action cannot be undone.`,
        icon: 'warning',
    ...
    if (result.isConfirmed) {
        try {
            const response = await fetch(`{{ route('Admin.leave-types.index') }}/${leaveTypeId}`, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
            ...
            toastr.error('An error occurred while deleting the leave type', 'Error');
        }
    }
}
...
async function createDefaults() {
    const result = await Swal.fire({
        title: 'Create Default Leave Types?',
        text: 'This will create standard leave types (Sick Leave, Casual Leave, Annual Leave, etc.) for your company.',
        icon: 'question',
        showCancelButton: true,
        confirmButtonColor: '#28a745',
        cancelButtonColor: '#3085d6',
        confirmButtonText: 'Yes, create defaults',
        cancelButtonText: 'Cancel'
    });
    ...
    if (result.isConfirmed) {
        try {
            ...
            const response = await fetch('{{ route("Admin.leave-types.create-defaults") }}', {
                method: 'POST',
                body: formData,
                headers: {
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
            ...
            toastr.error('An error occurred while creating default leave types', 'Error');
        }
    }
}
